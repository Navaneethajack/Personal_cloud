...
def render_setup_instructions() -> None:
    ...
    st.markdown("Please configure the following values in `.streamlit/secrets.toml`:")
    for key in missing:
        st.markdown(f"- `{key}`")
...
def load_config() -> AppConfig:
    ...
    config = AppConfig(
        app_password=str(secrets.get("APP_PASSWORD", "")),
        b2_endpoint=str(secrets.get("B2_ENDPOINT", "")).rstrip("/"),
        b2_bucket_name=str(secrets.get("B2_BUCKET_NAME", "")),
        b2_key_id=str(secrets.get("B2_KEY_ID", "")),
        b2_application_key=str(secrets.get("B2_APPLICATION_KEY", "")),
        max_file_size_mb=_safe_int(
            secrets.get("MAX_FILE_SIZE_MB", DEFAULT_MAX_FILE_SIZE_MB),
            DEFAULT_MAX_FILE_SIZE_MB,
        ),
        free_storage_limit_gb=_safe_int(
            secrets.get("FREE_STORAGE_LIMIT_GB", DEFAULT_FREE_STORAGE_LIMIT_GB),
            DEFAULT_FREE_STORAGE_LIMIT_GB,
        ),
        db_path=str(secrets.get("DB_PATH", DEFAULT_DB_PATH)),
    )
