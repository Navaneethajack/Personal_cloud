...
def load_config() -> AppConfig:
    ...
    config = AppConfig(
        ...
        b2_endpoint=str(secrets.get("B2_ENDPOINT", "")).strip().rstrip("/"),
        b2_bucket_name=str(secrets.get("B2_BUCKET_NAME", "")).strip(),
        b2_key_id=str(secrets.get("B2_KEY_ID", "")).strip(),
        b2_application_key=str(secrets.get("B2_APPLICATION_KEY", "")).strip(),
