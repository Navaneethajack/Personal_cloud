...
from config import AppConfig, endpoint_looks_valid
...
def get_client(config: AppConfig):
    ...
    Raises B2Error (never a raw boto3/botocore exception) if the endpoint is
    malformed or the client otherwise fails to construct, so callers never
    need to guard against ValueError themselves.
    """
    if not endpoint_looks_valid(config.b2_endpoint):
        raise B2Error(
            f"B2_ENDPOINT is not a valid URL: '{config.b2_endpoint}'. "
            "It must look like https://s3.<region>.backblazeb2.com with no "
            "extra spaces, and the region must be copied exactly from your "
            "bucket's page in the Backblaze console."
        )
    try:
        return _cached_client(config.b2_endpoint, config.b2_key_id, config.b2_application_key)
    except Exception as exc:  # noqa: BLE001 - boto3/botocore raise several exception types here
        logger.exception("Failed to construct B2 client")
        raise B2Error(f"Could not create a Backblaze B2 client: {exc}") from exc
...
def _describe_client_error(exc: ClientError, action: str, object_key: str) -> str:
    """
    Turn a raw botocore ClientError into a specific, actionable message
    instead of a generic 'X failed' string. This mirrors the same
    code-to-message mapping check_connection() already does for
    head_bucket, applied consistently to every other B2 operation too --
    previously only check_connection() gave specific reasons; upload/
    download/delete all swallowed the real cause behind a generic message,
    which is exactly what made the original upload failure undiagnosable
    from the UI alone.
    """
    ...
    if code in ("InvalidAccessKeyId", "SignatureDoesNotMatch", "403", "AccessDenied"):
        return (
            f"{action} '{object_key}' was rejected: Backblaze B2 credentials are invalid "
            f"or lack permission for this bucket (code: {code}). Check B2_KEY_ID and "
            f"B2_APPLICATION_KEY, and that the key has Read+Write access to this bucket."
        )
    if code in ("NoSuchBucket", "404"):
        return (
            f"{action} '{object_key}' failed: bucket '{{bucket}}' was not found (code: {code}). "
            f"Check B2_BUCKET_NAME and that B2_ENDPOINT matches the bucket's actual region."
        )
    if code == "EntityTooLarge":
        return f"{action} '{object_key}' failed: file exceeds Backblaze B2's maximum object size (code: {code})."
    if code == "RequestTimeTooSkewed":
        return (
            f"{action} '{object_key}' failed: the server's clock is too far out of sync with "
            f"Backblaze B2 (code: {code}). This is a hosting-environment issue, not your credentials."
        )
    if code == "SlowDown":
        return f"{action} '{object_key}' failed: Backblaze B2 is rate-limiting this key (code: {code}). Try again shortly."

    # Unknown/unmapped code: still surface the REAL code and message rather
    # than a generic string, so the user isn't stuck guessing.
    return f"{action} '{object_key}' failed (code: {code}): {message}"
...
def upload_fileobj(config: AppConfig, file_obj: BinaryIO, object_key: str, content_type: str) -> None:
    ...
    try:
        ...
        detail = _describe_client_error(exc, "Upload", object_key).replace("{bucket}", config.b2_bucket_name)
        raise B2Error(detail) from exc
    except BotoCoreError as exc:
        logger.exception("B2 upload failed for key=%s", object_key)
        raise B2Error(
            f"Upload of '{object_key}' failed: could not reach Backblaze B2 "
            f"({type(exc).__name__}: {exc}). Check network connectivity and B2_ENDPOINT."
        ) from exc
