...
def _build_client(endpoint: str, key_id: str, application_key: str):
    ...
    Two B2 compatibility fixes are applied here:

    1. request_checksum_calculation / response_checksum_validation are forced
       to "when_required" because recent boto3/botocore versions default to
       "when_supported", which sends an extra CRC32 checksum via a chunked
       trailer on every request. B2 does not support that trailer format.

    2. The "Expect: 100-continue" header is removed from PutObject requests.
       boto3 adds this header for uploads so it can wait for the server's
       100 Continue before sending the body. B2's S3-compatible endpoint
       responds to this with a 400 IncompleteBody error and immediately
       closes the TCP connection, which then causes a BadStatusLine error
       on retry (urllib3 tries to reuse the dead connection). Unregistering
       the handler forces boto3 to send the full body without the handshake.
    """
    client = boto3.client(
        "s3",
    ...
    # Remove Expect: 100-continue from PutObject — B2 mishandles it.
    client.meta.events.unregister(
        "before-call.s3.PutObject", botocore.handlers.add_expect_header
    )
    return client
...
def upload_fileobj(config: AppConfig, file_obj: BinaryIO, object_key: str, content_type: str) -> None:
    """
    Upload a file-like object to B2 under the given object key.

    Uses put_object with the full body read into memory, rather than
    boto3's managed upload_fileobj/TransferManager. TransferManager can
    switch to a chunked/streaming request encoding for larger payloads,
    which is the same style of request that Backblaze B2's S3-compatible
    endpoint can reject outright (see the checksum note on _build_client).
    put_object with a plain bytes body has a known Content-Length and is
    sent as a single, plainly-signed request, which is the reliable path
    against B2. This keeps the whole file in memory only briefly, which is
    consistent with this app's documented max-file-size memory trade-off.
    """
    client = get_client(config)
    try:
        file_obj.seek(0)
        ...
        raise B2Error(f"Upload to Backblaze B2 failed for '{object_key}': {exc}") from exc
